...
export default {
  ...
  GET_BY_FILTER: asyncHandler(async (req, res, ModelSchema) => {
    // findById
    ...
    if (helpersExtension.isNotNull(id)) {
      ...
      // get data from db
      await ModelSchema.findById(id).then((rs) => {
        cache.set(id, rs);
  ...
  INSERT: asyncHandler(async (req, res, ModelSchema) => {
  ...
  //update function
  UPDATE: asyncHandler(async (req, res, ModelSchema) => {
    var model = req.body;
  ...
  UPDATE_MANY: asyncHandler(async (req, res, ModelSchema) => {
    // var model = req.body;
    // model.updated_at = new Date();
    // var filter = { _id: model._id };
    // var updateValues = { $set: model };
  ...
  //delete
  DELETE: asyncHandler(async (req, res, ModelSchema) => {
    // check ids
